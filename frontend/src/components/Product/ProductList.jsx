...
const ProductList = () => {
  ...
  const fetchProducts = async () => {
    try {
      const res = await axios.get(
        `http://localhost/backend/products/${page}/${limit}${
          sortBy ? `?sort=${sortBy}` : ""
        }`,
        {
          headers: {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      await axios.delete(`http://localhost/backend/products/${id}`, {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
        },
            ...
            {filtered.map((p, i) => (
              <tr key={p.id}>
                ...
                <td className="d-flex align-items-center gap-2">
                  <img
                    src={
                      p.images?.[0]?.base64 || "https://via.placeholder.com/32"
                    }
                    alt={p.name}
                    width="32"
                    height="32"
                ...
                <td>
                  <div className="d-flex gap-2">
                    ...
                    <button className="btn btn-light btn-sm">
                      <NavLink key={p.id} to={`/admin/products/update/${p.id}`}>
              ...
              {selectedProduct && (
                <div className="row">
                  <div className="col-md-4 text-center">
                    <img
                      src={
                        selectedProduct.images?.[0]?.base64 ||
                  ...
                  <div className="col-md-8">
                    ...
                    <p>
                      <strong>OS:</strong> {selectedProduct.os}
                    </p>
                    <p>
                      <strong>Price:</strong> ${selectedProduct.price}
                    </p>
                    <p>
                      <strong>Published:</strong> {selectedProduct.published}
                    </p>
                    <p>
